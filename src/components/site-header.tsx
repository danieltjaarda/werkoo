...
function NavDropdown({ label, links }: { label: string; links: { label: string; href: string }[] }) {
  return (
    <div className="group relative">
      <button
        type="button"
        className="flex items-center gap-1.5 rounded-lg px-3 py-2 text-basis font-medium text-ink transition hover:bg-brand-soft group-focus-within:bg-brand-soft"
      >
        {label}
        <ChevronDownIcon className="h-4 w-4 text-ink-soft transition-transform duration-200 group-hover:rotate-180 group-focus-within:rotate-180" />
      </button>
      <div className="invisible absolute left-1/2 top-full z-20 w-60 -translate-x-1/2 pt-2 opacity-0 transition group-hover:visible group-hover:opacity-100 group-focus-within:visible group-focus-within:opacity-100">
        <ul className="rounded-2xl border border-lijn bg-white p-2 shadow-paneel">
          {links.map((link) => (
            <li key={link.label}>
              <Link
                href={link.href}
                className="block rounded-xl px-3 py-2 text-basis text-ink transition hover:bg-brand-soft hover:text-brand-deep"
              >
                {link.label}
              </Link>
            </li>
      ...
      <div className="container-page flex h-[84px] items-center gap-8">
        ...
        >
          <Image
            ...
            width={250}
            height={50}
            // `priority` is in Next 16 vervangen door `preload`.
            preload
        ...
        <nav aria-label="Hoofdmenu" className="ml-auto hidden items-center gap-1 lg:flex">
          ...
          <Link
            href="/inloggen"
            className="rounded-lg px-3 py-2 text-basis font-medium text-ink transition hover:bg-brand-soft"
          >
            Inloggen
          </Link>
          <Link
